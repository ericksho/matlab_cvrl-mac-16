function mosaic = Bmv_mosaic2(im1, im2, show)
...
Ka = Ja>0;
Kb = Jb>0;
Km  = and(Ka,not(imerode(Ka,ones(1,45))));
Km1 = and(Ka,not(imerode(Ka,ones(1,90))));

%Kb = imdilate(Kb,ones(21,21));
Dm  = double(bwdist(not(Km1))).*double(Km);
Dm = Dm/max2(Dm);
...
J = Jb;
J(ia) = Ja(ia);
J(im) = double(Jb(im)).*(1-Dm(im))+double(Ja(im)).*Dm(im);


mosaic = uint8(round(J));
